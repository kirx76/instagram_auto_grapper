...
def get_media(bot, message, instagram_user, target):
    try:
        ...
        if target == 'posts':
            ...
            if len(new_posts) > 0:
                grap_posts(bot, message, instagram_user, valid_instagram_account, amount=len(new_posts))

        if target == 'stories':
            new_stories = get_new_stories(instagram_user, valid_instagram_account)
            if len(new_stories) > 0:
                grap_stories(bot, message, instagram_user, valid_instagram_account, amount=len(new_stories))

        if target == 'highlights':
            new_highlights = get_new_highlights(instagram_user, valid_instagram_account)
            if len(new_highlights) > 0:
                grap_highlights(bot, message, instagram_user, valid_instagram_account)
